...
Run `bats tests`, `shellcheck hooks/* lib/*`, and the Buildkite plugin linter before submitting changes. Tests make no live network requests. The plugin requires `mise` plus the standard Linux `find`, `sort`, and `mktemp` utilities at runtime.
