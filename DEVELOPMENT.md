...
Run `bats tests`, `shellcheck hooks/* lib/*`, and the Buildkite plugin linter before submitting changes. Tests make no live network requests. The plugin installs its pinned mise release and otherwise requires the standard Linux utilities listed in `plugin.yml`.
