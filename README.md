...
Add the plugin to a Buildkite Pipelines command step. The `workflow` attribute specifies the workflow file to import:
...
The command step must define a `key`. The importer uses this key to make the generated steps depend on the upload step. The plugin downloads and verifies the public `buildkite-gha` CLI, then uses it to translate and upload the workflow.
...
JavaScript actions run with exact Node.js versions `20.20.2` or `24.18.0`. The official Node.js binaries installed by mise require glibc 2.28 or newer. Shell-only workflows and the static `buildkite-gha` CLI do not have this requirement.
