...
For generated jobs that contain GitHub Actions, the runtime uses exact mise 2026.5.12. It uses a compatible executable already on `PATH`; when `PATH` has no compatible mise, it downloads the pinned official archive into the hosted cache and verifies both the archive and executable with embedded SHA-256 digests. An explicit `BUILDKITE_GHA_MISE` remains strict: it must name an absolute executable path to exact mise 2026.5.12. The plugin does not install or transport mise to generated jobs, and runtime agents do not need to preinstall it. Shell-only generated jobs skip mise setup entirely.
