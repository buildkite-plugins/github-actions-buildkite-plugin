...
The plugin uses mise and Go 1.26.5 to build Linux amd64, Darwin arm64, and Windows amd64 executables from that exact source, runs the executable native to the importer agent, and supplies the other runtimes to generated jobs. Testing experimental Windows jobs requires a source commit that supports Windows and a compatible Windows queue; the importer still runs on Linux amd64 or Darwin arm64. Source builds are for development only and do not use release checksums, attestations, or `minimum-release-age`.
