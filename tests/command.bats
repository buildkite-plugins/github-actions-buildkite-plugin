...
#!/usr/bin/env bash
if [[ "${1:-}" == --version ]]; then echo 'buildkite-gha 0.3.0'; exit; fi
printf 'executable=%s\n' "$0" >> "${MOCK_LOG:?}"
...
@test "installs and invokes importer with exact arguments" {
  ...
  grep -Fx 'https://github.com/buildkite/buildkite-gha/releases/download/v0.3.0/buildkite-gha_Linux_x86_64.tar.gz' "$MOCK_LOG"
  grep -Fx 'https://github.com/buildkite/buildkite-gha/releases/download/v0.3.0/checksums.txt' "$MOCK_LOG"
...
@test "accepts a leading v and rejects version injection" {
  export BUILDKITE_PLUGIN_GITHUB_ACTIONS_VERSION=v0.3.0
  run "$REPO/hooks/command"
  [ "$status" -eq 0 ] || { echo "$output"; false; }
...
@test "rejects unexpected content and symlinks before extraction" {
  ...
  [ ! -e "$BUILDKITE_GITHUB_ACTIONS_PLUGIN_CACHE_ROOT/v0.3.0/Linux_x86_64/evil" ]
...
@test "replaces a tampered cached archive before execution" {
  run "$REPO/hooks/command"; [ "$status" -eq 0 ]
  cache="$BUILDKITE_GITHUB_ACTIONS_PLUGIN_CACHE_ROOT/v0.3.0/Linux_x86_64/buildkite-gha_Linux_x86_64.tar.gz"
  mkdir "$TMP/tampered"
...
@test "uses an attached hosted cache volume" {
  ...
  [ -f "$MISE_HOSTED_CACHE_VOLUME_ROOT/github-actions-buildkite-plugin/v0.3.0/Linux_x86_64/buildkite-gha_Linux_x86_64.tar.gz" ]
...
@test "falls back to the agent cache when the hosted volume is unavailable" {
  ...
  [ -f "$BUILDKITE_AGENT_DATA_PATH/cache/github-actions-buildkite-plugin/v0.3.0/Linux_x86_64/buildkite-gha_Linux_x86_64.tar.gz" ]
}
...
@test "continues when the verified CLI archive cannot be cached" {
  run "$REPO/hooks/command"
  [ "$status" -eq 0 ] || { echo "$output"; false; }
  rm -rf "$BUILDKITE_GITHUB_ACTIONS_PLUGIN_CACHE_ROOT/v0.3.0"
  cat > "$TMP/bin/mktemp" <<'EOF'
#!/usr/bin/env bash
if [[ "$*" == *'/v0.3.0/.Linux_x86_64.'* ]]; then exit 1; fi
exec "${REAL_MKTEMP:?}" "$@"
EOF
  ...
  grep -Fx 'upload --runtime-queue hosted .github/workflows/ci.yml' "$MOCK_LOG"
  [ ! -e "$BUILDKITE_GITHUB_ACTIONS_PLUGIN_CACHE_ROOT/v0.3.0/Linux_x86_64" ]
}
...
#!/usr/bin/env bash
destination="${!#}"
if [[ "$*" == *'/cache/v0.3.0/Linux_x86_64/buildkite-gha_Linux_x86_64.tar.gz'* ]]; then
  printf 'partial\n' > "$destination"
  exit 1
fi
exec "${REAL_CP:?}" "$@"
...
@test "concurrent installs converge on one valid cache" {
  ...
  wait "$first"
  wait "$second"
  destination="$BUILDKITE_GITHUB_ACTIONS_PLUGIN_CACHE_ROOT/v0.3.0/Linux_x86_64"
